...
function [g,dG] = NeuralCost(z,consts)
    ...
    for i=1:N
        ...
        NeuralFun = @(z) nnPredict(Theta1,Theta2,Theta3,Theta4,z);
        ...
        g = g + g_i;                
              ...
              delta = 1e-8;
              dG = zeros(numel(xi), 2*(numel(xi)+numel(ui)));
              for j=1:numel(xi)
                  dx = zeros(numel(xi),1);
                  dx(j) = delta;
                  dGx_i_j = NeuralFun(xi + dx, ui, xip1, uip1, dt) - h_i;
                  dGx_ip1_j = NeuralFun(xi, ui, xip1 + dx, uip1, dt) - h_i;
                  dG(:,j) = dGx_i_j/delta;
